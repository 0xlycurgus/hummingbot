...
export const invalidPrivateKeyError: string =
  'The privateKey param is not a valid Near private key (base58 string worth 64 bytes).';

export const invalidPublicKeyError: string =
  'The spender param is not a valid Near address (base58 string worth 32 bytes).';

// test if a string matches the shape of an Near address
export const isPublicKey = (str: string): boolean => {
  return isBase58(str) && bs58.decode(str).length == 32;
};

// given a request, look for a key called address that is an Near address
