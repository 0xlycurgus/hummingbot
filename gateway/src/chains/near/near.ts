...
import { TransactionResponseStatusCode } from './near.requests';
...
export class Near {
  ...
  public static getInstance(): Near {
    if (!Near._instance) {
      Near._instance = new Near();
    }

    return Near._instance;
  }

  public static getConnectedInstances(): { [name: string]: Near } {
    return { solana: Near._instance };
  }

  public static reload(): Near {
    Near._instance = new Near();
    return Near._instance;
  }
  ...
  async encrypt(privateKey: string, password: string): Promise<string> {
    ...
    const keyMaterial = await Near.getKeyMaterial(password);
    const keyAlgorithm = {
      name: 'PBKDF2',
      salt: salt,
      iterations: 500000,
    ...
    const key = await Near.getKey(keyAlgorithm, keyMaterial);
